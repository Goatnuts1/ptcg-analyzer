...
| Effects | `src/engine/effects.py` — primitives + registries, **~84 cards** each tested vs card text | ✅ working |
| Decks | `dragapult`, `charizard_xy` (tournament lists) + `raging_bolt`, `gardevoir`, `colorless`, `fire`, `fighting`, `dark`, `metal`, `water`, `greninja`, `beedrill`, `crustle` (13 total) | ✅ `DECKS` registry |
...
Full test suite: **30 suites green.**

## Cards implemented (~84, each unit-tested)
...
- **Grass:** **Beedrill ex** (Rumbling Bees — 110× per Beedrill, a scaling swarm,
  `beedrill` deck) and **Crustle** (Mysterious Rock Inn — walls the opponent's ex
  attackers) + Dwebble/Brambleghast (`crustle` control). Both are real-meta Fighting
  counters; both are weak to Fire ×2.
