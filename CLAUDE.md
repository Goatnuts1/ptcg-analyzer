...
- Effect system: done + tested. `effects.py` = primitives + registries for attacks,
  abilities, Trainers, Tools, Special Energy. **~84 distinct cards implemented**,
...
- Decks (`decks.py`): two faithful tournament lists (`dragapult`, `charizard_xy`)
  plus eleven built archetypes (`raging_bolt`, `gardevoir`, `colorless`, `fire`,
  `fighting`, `dark`, `metal`, `water`, `greninja`, `beedrill`, `crustle`), in the `DECKS` registry;
  `load_deck(db, name)`.
